...
<acme:form>
	...
	<acme:input-textbox code="inventor.patronage.form.label.name"
		path="patronName" readonly="true"/>
	<acme:input-textbox code="inventor.patronage.form.label.surname"
		path="patronSurname" readonly="true" />
	<acme:input-email code="inventor.patronage.form.label.email"
		path="patronEmail" readonly="true"/>

	<jstl:choose>
		<jstl:when test="${ status=='PROPOSED' }">
			<acme:submit code="inventor.patronage.form.button.update"
				action="/inventor/patronage/update?id=${ patronageId }" />
		</jstl:when>
	</jstl:choose>
