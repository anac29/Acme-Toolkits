...
<acme:form>
	...
	<acme:input-moment code="patron.patronage.form.label.startMomentDate"
		path="startMomentDate" />
	<acme:input-moment code="patron.patronage.form.label.finalMomentDate"
		path="finalMomentDate" />
	<acme:input-url code="patron.patronage.form.label.link" path="link" />

	<hr>
	<h3>
		<acme:message code="patron.patronage.form.label.title" />
	</h3>
	<jstl:choose>

		<jstl:when	test="${acme:anyOf(command,'show, update, delete, create, publish') && published==false}">
			<acme:input-select code="patron.patronage.form.label.inventor"
				path="inventorId">
				<jstl:forEach items="${inventors}" var="inventor">
					<acme:input-option
						code="${inventor.getUserAccount().getUsername()}"
		...
		<jstl:when test="${command=='show'}">
			...
			<acme:input-textbox code="patron.patronage.form.label.name"
				path="inventorName" />
			<acme:input-textbox code="patron.patronage.form.label.surname"
				path="inventorSurname" />
			<acme:input-email code="patron.patronage.form.label.email"
				path="inventorEmail" />
	...
	<jstl:choose>
		<jstl:when
			...
			<acme:submit code="patron.patronage.form.button.publish"
